...
public class MovingPlatform : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (Vector3.Distance(transform.position, movement.startPosition) <= 0.01f){
			if (!atStart){
				waitingTimer = 0;
				atStart = true;
			}
		...
		else if (Vector3.Distance(transform.position, movement.endPosition) <= 0.01f){
			if (atStart){
				waitingTimer = 0;
				atStart = false;
			}
		...
		if (waitingTimer >= movement.waitingTimeBeforeMoving){
			...
			if (atStart){
				transform.position = Vector3.MoveTowards(transform.position, movement.endPosition, movement.movementSpeed * Time.deltaTime);
			}
			//moving to start position (if we are at the end)
			else {
				transform.position = Vector3.MoveTowards(transform.position, movement.startPosition, movement.movementSpeed * Time.deltaTime);
			}
